...
| **Effort profiles** | `fleet/core/effort_profiles.py` | Dispatch rate and model selection | **KEEP + CONNECT** — effort profiles control fleet intensity. The control surface (Work Mode, Cycle Phase, Backend Mode) extends this. The immune system respects the current effort profile when making decisions. |
