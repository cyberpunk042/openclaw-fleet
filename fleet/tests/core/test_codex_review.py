...
def test_trigger_trainee():
    yes, reason = should_trigger_review("trainee")
    assert yes
    assert "review needed" in reason


def test_no_trigger_expert():
    yes, reason = should_trigger_review("expert")
    assert not yes
    assert "does not require" in reason


def test_no_trigger_standard_tier():
    yes, reason = should_trigger_review("standard")
    assert not yes


def test_trigger_force():
    yes, reason = should_trigger_review("expert", force=True)
    assert yes
    assert "forced" in reason


def test_trigger_community():
    yes, _ = should_trigger_review("community")
    assert yes


def test_trigger_hybrid():
    yes, _ = should_trigger_review("hybrid")
    assert yes
...
def test_command_adversarial():
    assert review_command(adversarial=True) == CODEX_ADVERSARIAL_CMD


def test_command_standard_review():
    assert review_command(adversarial=False) == CODEX_REVIEW_CMD
...
def test_request_to_agent_instruction():
    req = CodexReviewRequest(
        pr_number=42, repo="user/repo",
        confidence_tier="trainee",
